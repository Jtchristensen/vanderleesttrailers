 ...
 *
 * This file is the backend copy, imported by both the admin API (which persists
 * the result) and the migration script. The frontend keeps a TypeScript twin at
 * frontend/src/app/pipes/trailer-title.pipe.ts — the two are verified against a
 * shared case table, so any change here must be mirrored there.
 */
 ...
 */
export function gvwrLabel(gvwr) {
 ...
 */
export function extractVariant(name, trailer) {
...
export function composeTitle(trailer) {
  const t = trailer || {};
  const parts = [
    t.year,
    t.make,
